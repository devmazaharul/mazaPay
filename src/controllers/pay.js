...
const senMoney=async(req,res,next)=>{
  try {
    ...
    if(!reciverEmail || !reciveAmount) throw AppError("Invalid reciver email or amount");
      if(!reciveAmount || reciveAmount<AppConfigaration.MINIMUM_TRANSACTION_AMOUNT) throw AppError("Invalid amount min "  + AppConfigaration.MINIMUM_TRANSACTION_AMOUNT)
    const currentUsr=req?.currentUserInfo;

    const responce=await payService.sendService({currentUsr,reciverEmail,reciveAmount})
    if(responce?.status){
...
const singletransaction=async(req,res,next)=>{
  try {
    ...
    if(!currentUsr) throw AppError("Invalid user token");
      const responce=await payService.getTransactionDetails({currentUsr,trxId});
      if(responce?.status==200){
        res.status(200).json(responce)
      }
