...
const accountCreateService = async ({ name, email, address }) => {
  try {
...
const getnarateApiKeyService=async({currentUser,marcentName})=>{
  try {
    ...
    if(findApiLen.apiKeyLimit==0) throw AppError("You can not generate more than 3 API keys");
    const findApiKey=await ApiKeyModel.findOne({marchenId:_id,marcentName});
    if(findApiKey) throw AppError("You already generated API key for this marcent");
